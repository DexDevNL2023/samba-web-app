...
export class NotificationCrudComponent implements OnInit {
  ...
  selectedItem: Notification; // Élément de type Notification actuellement sélectionné ou en cours de modification
  selectedItems: Notification[] = []; // Tableau d'éléments de type Notification sélectionnés
  ...
  branches: EntityByBranch<Notification>[] = [
      {
          name: 'Branch A',
          partenaires: [
              {
  ...
  constructor(
    ...
    private service: NotificationService, // Service pour les opérations CRUD génériques
  ...
  loadAccounts(): void {
    this.service.getAllAccounts().subscribe((accounts: Account[]) => {
  ...
  protected calculateTotalSubscriptions(branch: EntityByBranch<Notification>): number {
  ...
  protected editItem(item: Notification) {
  ...
  protected deleteItem(item: Notification) {
  ...
  protected confirmDelete() {
    ...
    this.service.delete((this.selectedItem as any).id).subscribe(() => { // Supprime l'élément via le service
      ...
      this.selectedItem = {} as Notification; // Réinitialise l'élément
    });
  }
